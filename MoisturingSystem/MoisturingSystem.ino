...
void populateScreens(SystemState* state, MSScreen* screens) {
  ...
  screens[MS_INTERVALS_SETTINGS_SCREEN].drawUI = &drawIntervalsSettingsScreen;
  screens[MS_INTERVALS_SETTINGS_SCREEN].handleButtons = &handleIntervalsSettingsScreen;

  screens[MS_PUMP_INTERVALS_SETTINGS_SCREEN].drawUI = &drawPumpIntervalsSettingsScreen;
  screens[MS_PUMP_INTERVALS_SETTINGS_SCREEN].handleButtons = &handlePumpIntervalsSettingsScreen;

  screens[MS_SENSOR_INTERVALS_SETTINGS_SCREEN].drawUI = &drawSensorIntervalsSettingsScreen;
  screens[MS_SENSOR_INTERVALS_SETTINGS_SCREEN].handleButtons = &handleSensorIntervalsSettingsScreen;

  screens[MS_STATS_SCREEN].drawUI = &drawStatisticsScreen;
  screens[MS_STATS_SCREEN].handleButtons = &handleStatisticsScreen;
}
...
void drawStatisticsScreen(SystemState* state, Action* a) {
  GFXcanvas16 mainCanvas = GFXcanvas16(SCREEN_WIDTH, SCREEN_HEIGHT);
  initCanvas(&mainCanvas);
...
void handleStatisticsScreen(SystemState* state, int buttonValue) {
  if (buttonValue > BUTTON_1_LOW && buttonValue < BUTTON_1_HIGH) {
    (*state).scr = MS_HOME_SCREEN;
  }
}
...
void drawSensorIntervalsSettingsScreen(SystemState* state, Action* a) {
  GFXcanvas16 mainCanvas = GFXcanvas16(SCREEN_WIDTH, SCREEN_HEIGHT);
  initCanvas(&mainCanvas);
...
void handleSensorIntervalsSettingsScreen(SystemState* state, int buttonValue) {
  preferences.begin(MS_PREFERENCES_ID, false);
  if (buttonValue > BUTTON_1_LOW && buttonValue < BUTTON_1_HIGH) {
    (*state).scr = MS_INTERVALS_SETTINGS_SCREEN;
  }
    ...
    settings.sid = nv;
    preferences.putInt(MS_SENSOR_INTERVAL_DRY, nv);
  }
  else if (buttonValue > BUTTON_3_LOW && buttonValue < BUTTON_3_HIGH) {
    int step = 5000;
    ...
    settings.siw = nv;
    preferences.putInt(MS_SENSOR_INTERVAL_PUMPING, nv);
  }
  preferences.end();
}

void drawPumpIntervalsSettingsScreen(SystemState* state, Action* a) {
  GFXcanvas16 mainCanvas = GFXcanvas16(SCREEN_WIDTH, SCREEN_HEIGHT);
  initCanvas(&mainCanvas);
  char pm1[20], pm2[20];
...
void handlePumpIntervalsSettingsScreen(SystemState* state, int buttonValue) {
  preferences.begin(MS_PREFERENCES_ID, false);
  if (buttonValue > BUTTON_1_LOW && buttonValue < BUTTON_1_HIGH) {
    (*state).scr = MS_INTERVALS_SETTINGS_SCREEN;
  }
    ...
    settings.pd = nv;
    preferences.putInt(MS_PUMP_MAX_DURATION, nv);
  }
  else if (buttonValue > BUTTON_3_LOW && buttonValue < BUTTON_3_HIGH) {
    int minute = 60000;
    ...
    settings.pi = nv;
    preferences.putInt(MS_PUMP_REACT_INT_DURATION, nv);
  }
  preferences.end();
}
